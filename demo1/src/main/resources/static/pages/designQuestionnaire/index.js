...
const onAddQuestion = (type) => {
  ...
  $('#problem').append(ele)
  problem.push({ problemName: '', mustAnswer: true, option: [{}] ,problemId: '', problemType: '', leftTitle:''})
...
const handleEditFinish = () => {
  ...
  problem.forEach(function (question,index){
    let params_question = {
     ...
     $.ajax({
       url: 'http://172.22.8.245:8089' + '/admin/addQuestionInfo',
       type: "POST",
    ...
    question.option.forEach(function(temp,index){
      ...
      $.ajax({
        url: 'http://172.22.8.245:8089' + '/admin/addOptionInfo',
        type: "POST",
